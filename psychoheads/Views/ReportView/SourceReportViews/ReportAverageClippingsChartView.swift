//
//  ReportAverageClippingsChartView.swift
//  psychoheads
//
...
struct ReportAverageClippingsChartView: View {
        ...
        if #available(iOS 17.0, *) {
            Chart {
                ForEach(sourceModel.averageClippingsPerSource, id: \.title) { data in
                    BarMark(
                        x: .value("Source Title", data.title),
                        y: .value("Average Clippings", data.averageClippings)
                    )
                    ...
                    if let selectedSourceTitle, selectedSourceTitle == data.title {
                        RuleMark(x: .value("Selected", selectedSourceTitle))
...
struct ReportAverageClippingsChartView_Previews: PreviewProvider {
